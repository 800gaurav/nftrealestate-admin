...
const SuspendedUsers = () => {
  ...
  const fetchSuspendedUsers = async () => {
    try {
      const res = await fetchData({ url: '/api/v1/admin/user/suspended-users' });
      if (res?.user) {
        setUsers(res.user);
        setFiltered(res.user);
      } else {
            ...
            {currentItems.map((user, index) => {
              ...
              return (
                <CTableRow key={user._id}>
                  ...
                  <CTableDataCell>{user.userId} ({user.name})</CTableDataCell>
                  <CTableDataCell>{user.phone}</CTableDataCell>
                  <CTableDataCell>{user.email}</CTableDataCell>
                  <CTableDataCell>${user.walletBalance || 0}</CTableDataCell>
                  <CTableDataCell>${user.fundBalance || 0}</CTableDataCell>
