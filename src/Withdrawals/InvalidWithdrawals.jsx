...
function InvalidWithdrawals() {
  ...
  const fetchWithdrawals = async () => {
    try {
      const res = await fetchData({ url: '/api/v1/user/payment/withdraw-history?type=rejected' });
      if (res?.data) {
            ...
            {currentItems.map((item, index) => {
              ...
              return (
                <CTableRow key={item._id}>
                  ...
                  <CTableDataCell>
                    <span className={`badge bg-${item.status === 'pending' ? 'warning' : 'success'}`}>
                      {item.status}
                    </span>
                  </CTableDataCell>
                  <CTableDataCell>
...
export default InvalidWithdrawals
